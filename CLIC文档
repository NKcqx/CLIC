...
我们设计和实现了`CLIC`，一个跨平台的处理框架，旨在解决上述问题。它会针对一个的工作流任务，自动化地为其选择最优的一个甚至多个平台来协同处理。对于组成一个工作流的多个算子，`CLIC`会根据算子之间的关系，寻找优化机会，动态对算子进行调度、融合，选择合适的执行平台，并部署在`k8s`上运行。`CLIC`被设计为具有着高可扩展性，能够非常方便地添加更多的算子和新的计算平台，并且能够灵活添加优化策略，用户只需要使用`CLIC`提供的（和具体平台无关的）API来构建执行计划即可。
...
`CLIC`系统提供一个中间语言描述/可视化界面的抽象，用户可利用系统中已实现的算子，用其构建执行特定任务的工作流。系统分为三个主要模块，分别是逻辑方案生成层，物理方案生成层，以及平台部署层。
...
[![CLIC执行流程](https://s1.ax1x.com/2020/07/14/UUQPIg.md.png)](https://imgchr.com/i/UUQPIg)
...
Core是CLIC的核心模块，其中定义了Operator和Channel这两个构成Workflow的重要部件。它的主要功能为：
...
1. Docker Image：每个计算平台都将被封装为Docker Image，在实际运行时被部署到K8S集群中，并由CLIC将要运行的Physical Plan传入对应平台的Docker Image中
2. 平台的配置文件：CLIC对不同计算平台的使用方法是通过读取平台对应的配置文件获得的，例如平台的计算性能、运行Docker Image所需的输入输出参数、环境变量等
...
Logical DataType是CLIC对所有计算平台上的数据类型做的统一抽象。因为不同计算平台上对数据类型的描述有所不同，例如32位 *int* 型数据在 *Java* 中为 "int" ；而在 C 中为 "int32_t"。因此为了在构建Workflow时能统一描述数据，CLIC将常见的数据类型做了抽象，称为Logical DataType。

CLIC目前所支持的逻辑数据类型如下表所示。这些数据类型都为抽象说明，使用于Logical Operator中，每个Logical Operator的输入输出类型都必须为以下抽象数据类型，系统按照名字进行识别。针对于每一个Physical Operator来说，均有所实现语言中与简单类型一一对应的数据类型，如INT64对应C语言中的int64_t，对应Java语言中的long。
...
Physical DataType 则是 Logical DataType 对应到具体计算平台上的数据类型。该表列出了CLIC在各平台上的Physical Data Type，以及他们所支持的相应的Logical Data Type。需要指出的是一个Physical Data Type可以对应着多个Logical Data Type，比如RDD可以用来实现Matrix、Array、KeyValue、Relation等复杂数据类型。在系统实现中，如果一个Physical Operator的输入输出数据类型能够对应于其Logical Operator中相应的类型，那么该Physical Operator就可以用来Materialize该Logical Operator。CLIC会存储不同Physical DataType间的转换函数，在进行跨平台数据传输时，CLIC会在上述对照表中查找两个不同的平台上的数据类型，并对数据使用对应的转换函数，以此完成数据格式转换。
...
4. **输入-输出数据的 Key Pair**：当Source - Target Operator存在依赖关系，即Target Operator所需的输入数据是Source Operator的输出数据时，需要使用一个Key Pair声明上述关系。Key Pair的第一个Key为Source Operator输出数据的Key，第二个Key 为Target Operator输入数据的Key
...
Logical Channel仅描述了Operator间的链接关系和两端Operator要传输的数据，并不涉及两端Operator的数据格式、数据传输所使用的协议（例如UDP, RDMA）。除此之外，当Logical Operator被一一映射到不同平台上成为Physical Operator后，相邻的两个Physical Operator很有可能处在不同的计算平台，而不同平台所使用的数据类型、存储方式也是不同的，此时称两个Operator所使用的数据是异质的，而异质数据的传输需要额外进行格式转换。因此，对于Physical Operator间的链接需要使用Physical Channel。

Physical Channel用于表达Physical Operator之间的关系，它在Logical Channel的基础上还包括了：对异质数据进行格式转换、切换不同的数据传输方式、生成相关参数等功能。这些功能实际上是由不同 （physical）Operator完成的，例如 *Datatype Conversion Operator* 负责转换不同的数据类型、*Storage Conversion Operator* 负责转换数据的存储格式（row-wise / col-wise）、*Transmission Operator*负责选择不同的数据传输方式，如使用文件I/O、Socket传输、RDMA等。Physical Channel通过组合以上三类不同功能的Operator来处理各类异质数据的传输和读写问题。
...
4. **Transmission Operator**：RDMA、Socket、HDFS、内存
5. **Datatype Conversion Operator**：负责数据类型转换的Operator
6. **Storage Conversion Operator**：负责数据存储格式转换的Operator
7. **Transmission Operator**：负责对使用数据使用不同传输方式的Operator
8. **相关参数**：传输所需的控制信息
...
由用户编写的Workflow其结构不一定是最优的，因此在实际运行前CLIC首先需要对Workflow进行优化重构。CLIC主要使用了以下几种优化策略。

**Operator Fusion**

Operator fusion基于的设想是，通过融合两个或多个`Operator`来减少`Operator`之间数据传输、封装带来的性能损失。

以 `map` fusion为例：两个相邻的 `map ` Operator 分别执行了不同的`UDF`，但由于是两个`Operator`，不仅在优化时需要CLIC分别计算代价并选择平台，而且在运行时需要进行两次封装、解封。因此我们合并两个`map` Operator，即将两段`UDF`合并放到一个`map` Operator中来省去上述多余的操作。如下图所示：

<img src="https://s1.ax1x.com/2020/07/16/UB0vb6.png" alt="map operator fusion.png" style="zoom: 67%;" />

**Operator Pushdown / pullup**

不同的`Operator`对数据元素的处理、性能消耗上有各自的特点，例如 `filter`不改变元素的值仅改变集合中元素的数量、`join`改变元素的值、数量且计算代价高昂、`sort`仅改变元素的相当位置，计算代价取决于数据量。

因此一个朴素的想法就是，在Logical Plan中将诸如`filter`的运算符进行pullup（更早计算），将`sort`、`join`运算符进行pushdown（更晚运算），这样可做的在不影响计算结果的前提下尽量减少运算时间。如下图所示：
...
而CLIC会针对当前任务，为每个Logical Operator从所有平台的实现中选择最优实现。以上图中经过平台映射后的Workflow为例，为其选择了最优平台后将得到类似下图的结构：
...
本节介绍了Workflow的概念及如何使用API构建Workflow，需要指出的是，**<u>Workflow的概念是完全面向用户的，即它的定义是为了让用户能够使用系统，和CLIC的具体实现没有关系</u>**。
...
本章以实现一个`MapOperator`为例，从Logical 到Physical 来介绍如何为CLIC引入一个新的Operator。

我们已知Operator分为Logical Operator和Physical Operator。Logical Operator是系统对运算符的抽象，用于提供给用户统一的描述；Physical Operator则是Logical Operator在各个平台上的具体实现（Implementation）。因此，为CLIC创建一个新的Operator时首先需要创建Logical Operator并向系统注册，然后再在某一平台上具体实现它。

#### 3.2.1 创建Logical Operator

CLIC中所有的Operator均是通过加载描述文件中的属性来创建的，因此区分不同的Operator仅通过在描述文件中提供不同的信息。创建Logical Operator主要需要以下几步：
...
* **Inputs**：`Operator`的输入数据列表，定义了输入数据的键名、数据类型等必要信息
* **Outputs**：`Operator`的输出参数列表，同理。

系统中的`Operator`类会解析上述XML文件，加载各个属性到当前创建的`Operator`实例中，此时便通过描述文件构造了一个有实际含义的Logical Operator。

**2. 向CLIC注册该Operator**

在提供了Map Operator的描述文件后，还需要向CLIC注册该Operator使其能够被系统调度使用。而系统是通过加载一个名为 *OperatorMapping.xml* 的配置文件来取得所有可用的Operator。所以，向CLIC注册新的Operator只需要在配置文件中添加对应信息即可， 在*OperatorMapping.xml* 中添加的内容如下：
...
**3. 使用CLIC的API创建Logical Operator**
...
2.  HashMap，要传入Map Operator的参数列表，CLIC在找到 "map"对应的Operator后会加载其描述文件并找到对应的参数列表；HashMap中传入的 "udfName"即为在*MapOperator.xml*中添加的  "parameter"字段
...
> 需要指出的是，具体平台上的代码实现和CLIC是完全解耦的。CLIC实际并不关心Map在Spark上是如何实现的，例如其中的`execute`接口如何调用、`ParamsModel`是什么等。这里贴出代码仅是为了以Spark平台的实现为例展示整体开发流程。



**2. 编写Physical Operator的描述文件**

**CLIC对Physical Operator使用仅限于根据其描述文件中的属性来生成对应参数**，因此在Spark平台上实现了Physical的Map Operator后还需要向系统提供Physical Operator的描述文件。**该描述文件对应于Logical Operator的配置文件中的 `platforms.platform.path`字段**。上述代码片段对应的 *SparkMapOperator.xml* 文件的内容如下：
...
文件中的 `cost` 字段描述了MapOperator在Spark平台上的性能，CLIC会比较MapOperator在各个平台上的cost来选择是否将将 Map Operator映射到Spark平台。
...
在CLIC中，Visitor的使用主要是为了在DAG上进行多种操作的同时保持与DAG的解耦，例如平台选择、优化、转译等操作均可在不修改DAG（即Operator）的前提下完成。

#### 3.4.2 遍历DAG

CLIC提供了多种对DAG的遍历方式，包括：DFS、BFS、Topological，分别由不同的类来实现。以BFS遍历为例，它对应于 `BFSTraversal`，核心代码如下：
...
不同的 `Operator` 所执行的计算任务不同，对应的最佳平台也不同。**CLIC基于cost model综合考虑、计算任务的*selectivity*、平台配置等参数来选择最佳平台**。
...
CLIC从运行日志中学习cost model各个输入的权重参数，训练完成后便可用于平台选择。
...
在当前的视线中，一个计算平台（如Spark, MPI）在CLIC中是一个完整的、可独立运行的docker image；Image中不仅包含计算平台的运行环境，更重要的是还包含该平台实现的所有 (Physical) Operator。
...
因此CLIC的做法是：提前将一个平台上所有的`Operator`封装成为一个运算库，同平台环境一起封装到Image内，运算库由image内的 *Composer* 程序来调度。

TODO: 具体如何将Operator设计为运算库，CLIC如何通过命令行参数使用Operator，Composer如何接受参数并调度相应的Operator等，可参考[附录](#appendix)。

#### 3.6.2 CLIC中对不同平台的调用

在2.2和3.5节中均有提到，CLIC通过Image的描述文件来获知其对应平台的相关信息，如：平台名称、Image（在repository中）的地址、Image的入口命令、Image的系统参数等。

在CLIC生成了Physical Plan后，每个Physical Operator都被指派了具体的计算平台。此时，CLIC会根据描述文件，为相邻的、在同一计算平台上的Operator所组成的sub-plan添加一些必要的运行参数；该参数将连通sub-plan一起作为参数传入平台的Image中。

而平台的实际运行是基于Argo Workflow的，因此CLIC在为平台的Image生成了参数后，下一步就是将平台属性和参数一起，生成Argo Workflow所需的YAML文件以便部署到k8s。
...
CLIC基于[Argo Workflow](https://argoproj.github.io/argo/)来将所有计算任务部署到k8s，计算任务在Argo中被称为Workflow，而Workflow最终又以YAML文件的形式被提交到k8s。**因此，CLIC需要将Physical Plan数据结构转译为Workflow的形式，并输出为YAML文件，这通过`ArgoVisitor`来完成。**
...
spec:
  ...
  - name: my-flow
    dag:
      tasks:
      - name: Stage-SourceOperator
        template: java-template
        arguments:
          parameters:
          - name: javaArgs
            value: java -jar executable-java.jar --udfPath=/Users/jason/Desktop/TestSmallWebCaseFunc.class
              --dagPath=/tmp/irdemo_output/physical-dag-1370161291.yml
      - name: Stage-MapOperator
        template: spark-template
        arguments:
          parameters:
          - name: sparkArgs
            value: /spark-runner/bin/spark-submit --master=k8s://https://10.176.25.102:6443
              --deploy-mode=client --name=spark-running --class=fdu.daslab.executable.spark.ExecuteSparkOperator
              --conf=spark.executor.instance=5 --conf=spark.kubernetes.container.image=spark:v0
              executable-spark.jar --udfPath=/data/TestSmallWebCaseFunc.class --dagPath=/tmp/irdemo_output/physical-dag--1581354762.yml
        dependencies:
        - Stage-SourceOperator
      - name: Stage-ReduceByKeyOperator
        template: java-template
        arguments:
          parameters:
          - name: javaArgs
            value: java -jar executable-java.jar --udfPath=/Users/jason/Desktop/TestSmallWebCaseFunc.class
              --dagPath=/tmp/irdemo_output/physical-dag-1567013202.yml
        dependencies:
        - Stage-MapOperator
  - name: java-template
    inputs:
