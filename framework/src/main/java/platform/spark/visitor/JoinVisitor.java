...
public class JoinVisitor implements SparkVisitor {
    ...
    private JoinVisitor(Operator left, Operator right, Param param){
        ...
        this.equalJoinPredicatePair=new EqualJoinPredicatePair(predicate.substring(leftStart,leftEnd),predicate.substring(rightStart));
    }
    @Override
    ...
    public static JoinVisitor newInstance(Operator operator){
        ...
        Map<String, Param> inputParameterList=operator.getInput_data_list();
        Param inputParameter=inputParameterList.get("predicate");
        return new JoinVisitor(inputChannel.get(0).getSourceOperator(),inputChannel.get(1).getSourceOperator(),inputParameter);
    }
